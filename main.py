...
#creates database and saves changes
db.create_all()
db.session.commit()
...
def add_user():
  ...
  # checks for info and returns error if missing
  if not name:
  ...
  # query to return all villain names in database
  villain = Villain.query.filter_by(name=name).first()

  # if name is already in database it return error
  if villain:
    errors.append("Oops! A villain with that name already exists!")

  # only adds data to database if no errors
  if errors: 
